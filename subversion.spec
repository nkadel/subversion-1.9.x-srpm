%{?scl:%scl_package subversion}
%{!?scl:%global pkg_name %{name}}

# set to zero to avoid running test suite
%if %{?scl:1}0 &&  0%{?rhel} < 8 
# we miss rubygem(test-unit) on 6/7 (EPEL .. not on CBS)
%define make_check 0 
%else
%define make_check 1
%endif
...
Provides: %{?scl_prefix}svn = %{version}-%{release}
...
%package -n %{?scl_prefix}mod_dav_svn
...
Requires: %{?scl_prefix}subversion-libs%{?_isa} = %{version}-%{release}
%if %{?scl:1}0
BuildRequires: httpd24-httpd-devel > 2.4.6
%else
BuildRequires: httpd-devel >= 2.0.45
%endif

%description -n %{?scl_prefix}mod_dav_svn
...
%if %{?scl:1}0
%define svn_prefix /usr
# hack, configure scl-prefixes kde library path ... but not include one ? ..
export LDFLAGS="-L/usr/%{_lib}/kde4/devel"
%else
%define svn_prefix %{_prefix}
%endif


# hack, avoid: "Only one of PREFIX or INSTALL_BASE can be given.  Not both." 
# problem due to perl-homedir settings - remove for mock/koji !
%if %{?scl:1}0
export PERL5LIB=""
export PERL_MB_OPT=""
...
%if %{?scl:1}0
        --with-apxs=%{_httpd24_apxs} --disable-mod-activation \
        --with-apache-libexecdir=%{_httpd24_moddir} \
%else
        --with-apxs=%{_httpd_apxs} --disable-mod-activation \
        --with-apache-libexecdir=%{_httpd_moddir} \
%endif
        --disable-static --with-sasl=%{svn_prefix} \
        --with-libmagic=%{svn_prefix} \
        --with-gnome-keyring \
%if %{with_java}
        --enable-javahl \
        --with-junit=%{svn_prefix}/share/java/junit.jar \
%endif
%if %{with_kwallet}
        --with-kwallet \
%endif
...
%if %{?scl:1}0
mkdir -p ${RPM_BUILD_ROOT}{%{_httpd24_modconfdir},%{_httpd24_confdir}}
%else
...
cat %{pkg_name}.lang exclude.tools.files >> %{pkg_name}.files
...
%post
%if %{use_systemd}
%systemd_post %{?scl_prefix}svnserve.service
%else
/sbin/chkconfig --add %{?scl_prefix}svnserve 2>&1 ||:
%endif

%preun
%if %{use_systemd}
%systemd_preun %{?scl_prefix}svnserve.service
%else
if [ $1 = 0 ]; then
        /sbin/service %{?scl_prefix}svnserve stop > /dev/null 2>&1 || :
        /sbin/chkconfig --del %{?scl_prefix}svnserve > /dev/null 2>&1 || :
fi
%endif

%postun
%if %{use_systemd}
%systemd_postun_with_restart %{?scl_prefix}svnserve.service
%endif
...
%files -n %{?scl_prefix}mod_dav_svn
%if %{?scl:1}0
%config(noreplace) %{_httpd24_modconfdir}/*.conf
%{_httpd24_moddir}/mod_*.so
%doc example.conf
%else
...
%files perl
%{?_scl_root}%{perl_vendorarch}/auto/SVN
%{?_scl_root}%{perl_vendorarch}/SVN
